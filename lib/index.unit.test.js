...
_parseShim2.default.initialize('test');
...
beforeAll(function () {
  ...
  var bouquets = [];
  for (var i = 0; i < 10; i++) {
    bouquets.push(_index2.default.getClassInst('Bouquet'));
  }
  return _parseShim2.default.Object.saveAll(bouquets).then(function (objs) {
    savedBouquets = objs;
  });
});
...
var TheParseObj = _parseShim2.default.Object.extend('TheParseObj');
...
describe('parsimonious methods', function () {
  ...
  describe('toJsn', function () {
    test('returns the input when input is not a Parse object or a plain, non-null object', function () {
      ...
      expect(_index2.default.toJsn(mySymbol)).toEqual(mySymbol);
      expect(_index2.default.toJsn(undefined)).toEqual(undefined);
      expect(_index2.default.toJsn(null)).toEqual(null);
      expect(_index2.default.toJsn(true)).toEqual(true);
      expect(_index2.default.toJsn(2)).toEqual(2);
      expect(_index2.default.toJsn('abc')).toEqual('abc');
    });
    test('returns a shallow JSON representation of a Parse object', function () {
      expect(_index2.default.toJsn(aParseObj)).toBeEquivalentObject({
        roses: 'red',
        violets: 'blue',
        grass: 'green'
      });
    ...
    test('returns deep JSON representation of a plain object containing a plain, non-Parse object', function () {
      ...
      expect(_index2.default.toJsn(plainObj, true)).toBeEquivalentObject({
        foo: 'bar',
        domo: 'arigato',
        things: ['cow', 'pencil'],
        doohicky: {
    ...
    test('returns deep JSON representation of a plain object containing a Parse object', function () {
      ...
      expect(_index2.default.toJsn(someObj, true)).toBeEquivalentObject({
        foo: 'bar',
        domo: 'arigato',
        things: ['cow', 'pencil'],
        aParseObj: {
  ...
  describe('objPick', function () {
    test('gets some columns from a Parse object and returns them in a plain object', function () {
      expect(_index2.default.objPick(aParseObj, 'roses,grass')).toEqual({
        roses: 'red',
        grass: 'green'
      });
      expect(_index2.default.objPick(aParseObj, ['roses', 'grass'])).toEqual({
  ...
  describe('objSetMulti', function () {
    test('sets some columns on a Parse object from a plain object', function () {
      _index2.default.objSetMulti(aParseObj, {
        valley: 'big',
        river: 'deep'
      });
  ...
  describe('newQuery', function () {
    test('returns a query that finds all instances of a Parse class', function () {
      ...
      return _index2.default.newQuery('Bouquet').find().then(function (objs) {
        ...
        expect(_index2.default.isPFObject(objs[0], 'Bouquet')).toBe(true);
        expect(_index2.default.isPFObject(objs[9], 'Bouquet')).toBe(true);
    ...
    test('returns a query limited to first n instances of a Parse class', function () {
      ...
      return _index2.default.newQuery('Bouquet', { limit: 5 }).find().then(function (objs) {
        ...
        expect(_index2.default.isPFObject(objs[0], 'Bouquet')).toBe(true);
        expect(_index2.default.isPFObject(objs[4], 'Bouquet')).toBe(true);
  ...
  describe('getObjById', function () {
    test('gets a Parse object from db by id', function () {
      ...
      return aParseObj.save({ roses: 'red' }).then(function (savedObj) {
        ...
        expect(_index2.default.isPFObject(retrievedObj)).toBe(true);
  ...
  describe('getUserById', function () {
    test('gets a Parse user by id', function () {
      expect.assertions(1);
      var user = new _parseShim2.default.User({
        username: 'foo manchu',
        password: 'je9w83d',
        email: 'foo@bar.com'
      });
      return user.signUp().then(function (aUser) {
        ...
        expect(_index2.default.isPFObject(aUser, 'User')).toBe(true);
      });
    });
  });
  ...
  describe('Roles', function () {
    ...
    var roleACL = new _parseShim2.default.ACL();
    roleACL.setPublicReadAccess(true);
    var adminRole = new _parseShim2.default.Role("Administrator", roleACL);
    var modRole = new _parseShim2.default.Role("Moderator", roleACL);
    ...
    describe('userHasRole', function () {
      test('determines if a user has a single role', function () {
        ...
        return user.signUp().then(function (aUser) {
          return _index2.default.userHasRole(aUser, 'Administrator').then(function (hasRole) {
            expect(hasRole).toBe(false);
            return adminRole.getUsers().add(aUser).save().then(function () {
              return _index2.default.userHasRole(aUser, 'Administrator');
            });
          }).then(function (hasRole) {
            expect(hasRole).toBe(true);
            return modRole.getUsers().add(aUser).save().then(function () {
              return _index2.default.userHasRole(aUser, {
    ...
    describe('getRole', function () {
      test('returns Role object by name', function () {
        ...
        return _index2.default.getRole('Administrator').then(function (role) {
          expect(_index2.default.isPFObject(role, 'Role')).toBe(true);
        });
      });
    });
    ...
    describe('Many-to-Many with Join Tables', function () {
      ...
      describe('getJoinTableName', function () {
        test('returns a name for a table used to join two other tables; format: <first table name>2<second table name>', function () {
          expect(_index2.default.getJoinTableName('Employee', 'Company')).toBe('Employee2Company');
        });
      });
      ...
      describe('joinWithTable', function () {
        ...
        test('creates a new class of object for joining two other classes', function () {
          expect.assertions(5);
          return _parseShim2.default.Object.saveAll([Ship, Destroyer, Fleet]).then(function (savedObjs) {
            ...
            expect(_index2.default.isPFObject(joinObj, 'Ship2Fleet')).toBe(true);
            var joinedShip = joinObj.get('ship');
            var joinedFleet = joinObj.get('fleet');
            expect(_index2.default.isPFObject(joinedShip, 'Ship')).toBe(true);
            expect(joinedShip.id).toEqual(origObjs.Ship.id);
            expect(_index2.default.isPFObject(joinedFleet, 'Fleet')).toBe(true);
        ...
        test('creates a new class of object for joining two other classes with metadata describing their relationship', function () {
          ...
          return _index2.default.joinWithTable({ Destroyer: origObjs.Destroyer, Fleet: origObjs.Fleet }, { active: true }).then(function (joinObj) {
            expect(_index2.default.isPFObject(joinObj, 'Destroyer2Fleet')).toBe(true);
            expect(joinObj.get('active')).toBe(true);
            var joinedDestroyer = joinObj.get('destroyer');
            var joinedFleet = joinObj.get('fleet');
            expect(_index2.default.isPFObject(joinedDestroyer, 'Destroyer')).toBe(true);
      ...
      describe('unJoinWithTable', function () {
        ...
        test('removes document from a join table that points to two specific instances of Parse.Object', function () {
          ...
          return _parseShim2.default.Object.saveAll([Ship, Fleet]).then(function (savedObjs) {
            origObjs.Ship = savedObjs[0];
      ...
      describe('getJoinQuery', function () {
        ...
        test('returns a Parse.Query on a table that joins two subclasses of Parse.Object with pointers', function () {
          ...
          return _parseShim2.default.Object.saveAll([Ship, Fleet]).then(function (savedObjs) {
            ...
            return _index2.default.getJoinQuery({ Ship: joinObj.get('ship'), Fleet: joinObj.get('fleet') }, { select: 'active' }).first();
          }).then(function (joinObj) {
            expect(_index2.default.isPFObject(joinObj, 'Ship2Fleet')).toBe(true);
            expect(joinObj.get('active')).toBe(true);
            // TODO I can't get query.select to work. It might be parse-mockdb. expect(joinObj.get('position')).toBeUndefined()
            return _index2.default.getJoinQuery({ Ship: joinObj.get('ship'), Fleet: joinObj.get('fleet') }, { select: ['position'] }).first();
          }).then(function (joinObj) {
            expect(_index2.default.isPFObject(joinObj, 'Ship2Fleet')).toBe(true);
  ...
  describe('isPFObject', function () {
    test('determines if a variable is a Parse.Object', function () {
      expect(_index2.default.isPFObject(aParseObj)).toBe(true);
    });
    test('determines if a variable is a Parse.Object of a certain class', function () {
      expect(_index2.default.isPFObject(aParseObj, 'TheParseObj')).toBe(true);
    });
    ...
    test('ignores invalid "ofClass" parameter', function () {
      expect(_index2.default.isPFObject(aParseObj, 3)).toBe(true);
    });
  });
  ...
  describe('isUser', function () {
    test('determines if a variable is a Parse.User', function () {
      ...
      return user.signUp().then(function (aUser) {
        expect(_index2.default.isUser(aUser)).toBe(true);
      });
    });
    test('determines if a variable is a not a Parse.User', function () {
      expect(_index2.default.isUser(aParseObj)).toBe(false);
    });
  });
});