...
before(function () {
  ...
  return unsavedParseObj.save({
    ...
    var bouquetSaves = ints.map(function (n) {
      return _node4.default.getClassInst('Bouquet').save({ active: false, aNum: n });
    });
...
describe('toJsn', function () {
  it('returns the passed value when it is not a Parse object or a plain, non-null object', function () {
    ...
    expect(_node4.default.toJsn(mySymbol)).to.equal(mySymbol);
    expect(_node4.default.toJsn(undefined)).to.equal(undefined);
    expect(_node4.default.toJsn(null)).to.equal(null);
    expect(_node4.default.toJsn(true)).to.be.true;
    expect(_node4.default.toJsn(2)).to.equal(2);
    expect(_node4.default.toJsn('abc')).to.equal('abc');
  });
  it('returns a shallow JSON representation of a Parse object', function () {
    expect(_node4.default.toJsn(unsavedParseObj)).to.shallowDeepEqual({
      roses: 'red',
      violets: 'blue',
      grass: 'green'
    });
  ...
  it('returns deep JSON representation of a plain object containing a plain, non-Parse object', function () {
    ...
    expect(_node4.default.toJsn(plainObj, true)).to.shallowDeepEqual({
      foo: 'bar',
      domo: 'arigato',
      things: ['cow', 'pencil'],
      doohicky: {
  ...
  it('returns deep JSON representation of a plain object containing a Parse object', function () {
    ...
    expect(_node4.default.toJsn(someObj, true)).to.shallowDeepEqual({
      foo: 'bar',
      domo: 'arigato',
      things: ['cow', 'pencil'],
      aParseObj: {
...
describe('objPick', function () {
  it('gets some columns from a Parse object and returns them in a plain object', function () {
    expect(_node4.default.objPick(unsavedParseObj, 'roses,grass')).to.eql({
      roses: 'red',
      grass: 'green'
    });
    expect(_node4.default.objPick(unsavedParseObj, ['roses', 'grass'])).to.eql({
...
describe('objGetDeep', function () {
  it('Get an an object-type column from a Parse object and return the value of a nested key within it', function () {
    ...
    expect(_node4.default.objGetDeep(someObj, 'depts.accounting.employees[0].name')).to.equal('fred');
    expect(_node4.default.objGetDeep(someObj, 'depts.accounting.employees')).to.have.lengthOf(2);
  });
});
...
describe('newQuery', function () {
  it('returns a query that finds all instances of a Parse class', function () {
    return _node4.default.newQuery('Bouquet').find().then(function (objs) {
      ...
      expect(_node4.default.isPFObject(objs[0], 'Bouquet')).to.be.true;
      expect(_node4.default.isPFObject(objs[9], 'Bouquet')).to.be.true;
  ...
  it('returns a query limited to first n instances of a Parse class', function () {
    return _node4.default.newQuery('Bouquet', { limit: 5 }).find().then(function (objs) {
      ...
      expect(_node4.default.isPFObject(objs[0], 'Bouquet')).to.be.true;
      expect(_node4.default.isPFObject(objs[4], 'Bouquet')).to.be.true;
  ...
  it('returns a query that selects only a certain column to be returned', function () {
    return _node4.default.newQuery('Bouquet', { select: 'aNum' }).find().then(function (objs) {
...
describe('getObjById', function () {
  it('gets a Parse object from db by id', function () {
    ...
    return aParseObj.save({ roses: 'red' }).then(function (savedObj) {
      ...
      expect(_node4.default.isPFObject(retrievedObj)).to.be.true;
...
describe('getUserById', function () {
  it('gets a Parse user by id', function () {
    ...
    return user.signUp().then(function (aUser) {
      ...
      expect(_node4.default.isPFObject(aUser, 'User')).to.be.true;
    });
  });
});
...
describe('fetchIfNeeded, given a value <thing>, return a promise that resolves to', function () {
  ...
  it('fetched Parse.Object if thing is a pointer', function () {
    return _node4.default.fetchIfNeeded(savedBouquets[0].toPointer()).then(function (result) {
      ...
      expect(_node4.default.isPFObject(result)).to.be.true;
      expect(_node4.default.isPFObject(result, 'Bouquet')).to.be.true;
    });
  });
  it('thing if otherwise', function () {
    return expect(_node4.default.fetchIfNeeded('blah')).to.eventually.equal('blah');
  });
});
...
describe('Roles', function () {
  ...
  describe('userHasRole', function () {
    ...
    it('determines if a user has a single role', function () {
      return _node4.default.userHasRole(user, 'Administrator').then(function (hasRole) {
        expect(hasRole).to.equal(false);
        return adminRole.getUsers().add(user).save().then(function () {
          return _node4.default.userHasRole(user, 'Administrator');
        });
      }).then(function (hasRole) {
        expect(hasRole).to.be.true;
        return _node4.default.userHasRole(user, {
    ...
    it('reject when invalid "user" param', function () {
      return expect(_node4.default.userHasRole()).to.be.rejected;
    });
    it('reject when invalid "roles" param', function () {
      return expect(_node4.default.userHasRole(user)).to.be.rejected;
    });
  });
  ...
  describe('getRole', function () {
    it('returns Role object by name', function () {
      return _node4.default.getRole('Administrator').then(function (role) {
        expect(_node4.default.isPFObject(role, 'Role')).to.be.true;
      });
    });
  });
  ...
  describe('Many-to-Many with Join Tables', function () {
    ...
    describe('getJoinTableName', function () {
      it('returns a name for a table used to join two other tables; format: <first table name>2<second table name>', function () {
        expect(_node4.default.getJoinTableName('Employee', 'Company')).to.equal('Employee2Company');
      });
    });
    ...
    describe('joinWithTable', function () {
      ...
      it('creates a new class of object for joining two other classes', function () {
        return _node2.default.Object.saveAll([Ship, Destroyer, Fleet]).then(function (savedObjs) {
          ...
          expect(_node4.default.isPFObject(joinObj, 'Ship2Fleet')).to.be.true;
          var joinedShip = joinObj.get('ship');
          var joinedFleet = joinObj.get('fleet');
          expect(_node4.default.isPFObject(joinedShip, 'Ship')).to.be.true;
          expect(joinedShip.id).to.equal(origObjs.Ship.id);
          expect(_node4.default.isPFObject(joinedFleet, 'Fleet')).to.be.true;
      ...
      it('creates a new class of object for joining two other classes with metadata describing their relationship', function () {
        return _node4.default.joinWithTable({ Destroyer: origObjs.Destroyer, Fleet: origObjs.Fleet }, { active: true }).then(function (joinObj) {
          expect(_node4.default.isPFObject(joinObj, 'Destroyer2Fleet')).to.be.true;
          expect(joinObj.get('active')).to.be.true;
          var joinedDestroyer = joinObj.get('destroyer');
          var joinedFleet = joinObj.get('fleet');
          expect(_node4.default.isPFObject(joinedDestroyer, 'Destroyer')).to.be.true;
...
describe('isPFObject', function () {
  it('checks a valid Parse.Object', function () {
    expect(_node4.default.isPFObject(unsavedParseObj)).to.be.true;
  });
  it('checks Parse.Object of a certain class', function () {
    expect(_node4.default.isPFObject(unsavedParseObj, 'TheParseObj')).to.be.true;
  });
  it('ignores invalid "ofClass" parameter', function () {
    expect(_node4.default.isPFObject(unsavedParseObj, 3)).to.be.true;
  });
  ...
  it('returns false for pointer created locally as plain object', function () {
    expect(_node4.default.isPFObject({ __type: 'Pointer', className: 'HairBall', objectId: 'kjasoiuwne' })).to.equal(false);
  });
  it('returns false for pointer created with Parse.Object.toPointer', function () {
    expect(_node4.default.isPFObject(savedBouquets[0].toPointer())).to.equal(false);
  });
  it('returns true for Parse.Object subclass reference created with Parse.Object.createWithoutData', function () {
    expect(_node4.default.isPFObject(TheParseObj.createWithoutData('ihsd978h293'))).to.be.true;
  });
});

describe('isPointer', function () {
  it('should return false for scalars', function () {
    expect(_node4.default.isPointer('Schnauser')).to.equal(false);
    expect(_node4.default.isPointer(1)).to.equal(false);
  });
  it('should return false for non-qualifying objects', function () {
    expect(_node4.default.isPointer(null)).to.equal(false);
    expect(_node4.default.isPointer(savedParseObj)).to.equal(false);
    expect(_node4.default.isPointer(_node4.default.toJsn(savedParseObj))).to.equal(false);
    expect(_node4.default.isPointer({ __type: 'Pointer', className: 'HairBall' })).to.equal(false);
    expect(_node4.default.isPointer(TheParseObj.createWithoutData('ihsd978h293'))).to.equal(false);
  });
  it('should return true for qualifying objects', function () {
    expect(_node4.default.isPointer(savedParseObj.toPointer())).to.be.true;
    expect(_node4.default.isPointer({ className: 'HairBall', objectId: 'kjasoiuwne' })).to.be.true;
    expect(_node4.default.isPointer(_node4.default.toJsn(savedParseObj.toPointer()))).to.be.true;
    expect(_node4.default.isPointer(_node4.default.toJsn(savedParseObj.toPointer()))).to.be.true;
  });
});
...
describe('getPFObjectClassName', function () {
  ...
  it('returns valid class-name of a subclass of Parse.Object without leading underscore', function () {
    expect(_node4.default.getPFObjectClassName(unsavedParseObj)).to.equal('TheParseObj');
    expect(_node4.default.getPFObjectClassName('User')).to.equal('User');
    expect(_node4.default.getPFObjectClassName('_User')).to.equal('User');
    expect(_node4.default.getPFObjectClassName(user)).to.equal('User');
    expect(_node4.default.getPFObjectClassName('_Session')).to.equal('Session');
    expect(_node4.default.getPFObjectClassName({ naughty: 'object' })).not.to.be.ok;
    expect(_node4.default.getPFObjectClassName([1, 2, 3])).not.to.be.ok;
  });
});
...
describe('isUser', function () {
  ...
  it('determines if an object created with new Parse.User(...) -- but not yet saved -- is an instance of Parse.User', function () {
    expect(_node4.default.isUser(user)).to.be.true;
  });
  ...
  it('determines if an object created with new Parse.User(...) -- and then saved -- is an instance of Parse.User', function () {
    return expect(user.signUp().then(_node4.default.isUser)).to.eventually.equal(true);
  });
  it('determines if a pointer to a User is a not an instance of Parse.User', function () {
    expect(_node4.default.isUser(user.toPointer())).to.equal(false);
  });
  it('determines if a object is a not an instance of Parse.User', function () {
    expect(_node4.default.isUser(unsavedParseObj)).to.equal(false);
  });
});

describe('classStringOrSpecialClass', function () {
  it('converts the unchanged string if not a special Parse class', function () {
    expect(_node4.default.classStringOrSpecialClass('Horse')).to.equal('Horse');
  });
  it('converts "User" to Parse.User', function () {
    expect(_node4.default.classStringOrSpecialClass('User')).to.equal(_node2.default.User);
  });
  it('converts "Session" to Parse.Session', function () {
    expect(_node4.default.classStringOrSpecialClass('Session')).to.equal(_node2.default.Session);
  });
  it('converts "Role" to Parse.Role', function () {
    expect(_node4.default.classStringOrSpecialClass('Role')).to.equal(_node2.default.Role);
  });
});
...
describe('_toArray', function () {
  ...
  it('does not convert an array; just returns it', function () {
    expect(_node4.default._toArray(anArray)).to.equal(anArray);
  });

  it('converts a string to an array, splitting by commas if present', function () {
    expect(_node4.default._toArray('a,b,c')).to.eql(['a', 'b', 'c']);
  });

  it('converts any other type of value to an array with the value as the only item', function () {
    expect(_node4.default._toArray({ hello: 'there' })).to.eql([{ hello: 'there' }]);
    expect(_node4.default._toArray(88)).to.eql([88]);
  });
});