...
describe('parsimonious methods', function () {
  ...
  beforeAll(function () {
    ...
    return _node2.default.Object.saveAll(Array(10).fill(_node4.default.getClassInst('Bouquet', { active: false })), {
      success: function success(objs) {
        savedBouquets = objs;
        return new TheParseObj().save({ car: 'fast' }).then(function (obj) {
          savedParseObj = obj;
          return;
        });
      },
  ...
  describe('toJsn', function () {
    test('returns the passed value when it is not a Parse object or a plain, non-null object', function () {
    ...
    test('returns a shallow JSON representation of a Parse object', function () {
      expect(_node4.default.toJsn(unsavedParseObj)).toBeEquivalentObject({
        roses: 'red',
        violets: 'blue',
        grass: 'green'
      });
    ...
    test('returns deep JSON representation of a plain object containing a Parse object', function () {
      ...
      expect(_node4.default.toJsn(someObj, true)).toEqual({
        foo: 'bar',
        domo: 'arigato',
        things: ['cow', 'pencil'],
        aParseObj: {
  ...
  describe('objPick', function () {
    test('gets some columns from a Parse object and returns them in a plain object', function () {
      expect(_node4.default.objPick(unsavedParseObj, 'roses,grass')).toEqual({
        roses: 'red',
        grass: 'green'
      });
      expect(_node4.default.objPick(unsavedParseObj, ['roses', 'grass'])).toEqual({
  ...
  describe('objGetDeep', function () {
    test('Get an an object-type column from a Parse object and return the value of a nested key within it', function () {
      var someObj = _node4.default.getClassInst('Company', {
        depts: {
          accounting: {
            employees: [{
              name: 'fred',
      ...
      expect(_node4.default.objGetDeep(someObj, 'depts.accounting.employees[0].name')).toBe('fred');
      expect(_node4.default.objGetDeep(someObj, 'depts.accounting.employees')).toHaveLength(2);
    });
  });

  describe('objSetMulti', function () {
    test('sets some columns on a Parse object from a plain object', function () {
      _node4.default.objSetMulti(unsavedParseObj, {
        valley: 'big',
        river: 'deep'
      });
  ...
  describe('fetchIfNeeded, given a value <thing>, return a promise that resolves to', function () {
    test('thing if thing is a clean Parse.Object', function () {
      return _node4.default.fetchIfNeeded(savedBouquets[0]).then(function (result) {
        expect(result).toBe(savedBouquets[0]);
      });
    });
    test('fetched Parse.Object if thing is a dirty Parse.Object', function () {
      savedBouquets[0].set('active', true);
      return _node4.default.fetchIfNeeded(savedBouquets[0]).then(function (result) {
        expect(result.get('active')).toBe(false);
      });
    });
    test('fetched Parse.Object if thing is a pointer', function () {
      return _node4.default.fetchIfNeeded(savedBouquets[0].toPointer()).then(function (result) {
        expect(result).toBeDefined();
        expect(_node4.default.isPFObject(result)).toBe(true);
        expect(_node4.default.isPFObject(result, 'Bouquet')).toBe(true);
      });
    });
    test('thing if otherwise', function () {
      return expect(_node4.default.fetchIfNeeded('blah')).resolves.toEqual('blah');
    });
  });
  ...
  describe('getClass', function () {
    test('returns a subclass of Parse.Object', function () {
      var cls = _node4.default.getClass('Colors');
      expect(typeof cls === 'undefined' ? 'undefined' : _typeof(cls)).toBe('function');
      expect(cls.className === 'Colors').toBe(true);
    });
  });

  describe('getClassInst', function () {
    test('returns an instance of a subclass of Parse.Object', function () {
      var inst = _node4.default.getClassInst('Colors');
      expect(typeof inst === 'undefined' ? 'undefined' : _typeof(inst)).toBe('object');
  ...
  describe('isPFObject', function () {
    test('checks a valid Parse.Object', function () {
      expect(_node4.default.isPFObject(unsavedParseObj)).toBe(true);
    });
    test('checks Parse.Object of a certain class', function () {
      expect(_node4.default.isPFObject(unsavedParseObj, 'TheParseObj')).toBe(true);
    });
    test('ignores invalid "ofClass" parameter', function () {
      expect(_node4.default.isPFObject(unsavedParseObj, 3)).toBe(true);
    });
    test('checks a special sub-class of Parse.Object, like "User"', function () {
      expect.assertions(1);
      var user = new _node2.default.User({
        username: 'chuck biff',
        password: 'je9w83d',
    ...
    test('returns false for pointer created locally as plain object', function () {
      expect(_node4.default.isPFObject({ __type: 'Pointer', className: 'HairBall', objectId: 'kjasoiuwne' })).toBe(false);
    });
    test('returns false for pointer created with Parse.Object.toPointer', function () {
      expect(_node4.default.isPFObject(savedBouquets[0].toPointer())).toBe(false);
    });
    test('returns true for Parse.Object subclass reference created with Parse.Object.createWithoutData', function () {
      expect(_node4.default.isPFObject(TheParseObj.createWithoutData('ihsd978h293'))).toBe(true);
    });
  });

  describe('isPointer', function () {
    test('should return false for scalars', function () {
      expect(_node4.default.isPointer('Schnauser')).toBe(false);
      expect(_node4.default.isPointer(1)).toBe(false);
    });
    test('should return false for non-qualifying objects', function () {
      expect(_node4.default.isPointer(null)).toBe(false);
      expect(_node4.default.isPointer(savedParseObj)).toBe(false);
      expect(_node4.default.isPointer(_node4.default.toJsn(savedParseObj))).toBe(false);
      expect(_node4.default.isPointer({ __type: 'Pointer', className: 'HairBall' })).toBe(false);
      expect(_node4.default.isPointer(TheParseObj.createWithoutData('ihsd978h293'))).toBe(false);
    });
    test('should return true for qualifying objects', function () {
      expect(_node4.default.isPointer(savedParseObj.toPointer())).toBe(true);
      expect(_node4.default.isPointer({ className: 'HairBall', objectId: 'kjasoiuwne' })).toBe(true);
      expect(_node4.default.isPointer(_node4.default.toJsn(savedParseObj.toPointer()))).toBe(true);
      expect(_node4.default.isPointer(_node4.default.toJsn(savedParseObj.toPointer()))).toBe(true);
    });
  });
  ...
  describe('getPFObjectClassName', function () {
    ...
    test('returns valid class-name of a subclass of Parse.Object without leading underscore', function () {
      expect(_node4.default.getPFObjectClassName(unsavedParseObj)).toBe('TheParseObj');
      expect(_node4.default.getPFObjectClassName('User')).toBe('User');
      expect(_node4.default.getPFObjectClassName('_User')).toBe('User');
      expect(_node4.default.getPFObjectClassName(user)).toBe('User');
      expect(_node4.default.getPFObjectClassName('_Session')).toBe('Session');
      expect(_node4.default.getPFObjectClassName({ naughty: 'object' })).not.toBeDefined();
      expect(_node4.default.getPFObjectClassName([1, 2, 3])).not.toBeDefined();
    });
  });

  describe('isUser', function () {
    var user = new _node2.default.User();
    test('determines if an object created with new Parse.User(...) -- but not yet saved -- is an instance of Parse.User', function () {
      expect(_node4.default.isUser(user)).toBe(true);
    });
    test('determines if the same object with some attributes set is an instance of Parse.User', function () {
      _node4.default.objSetMulti(user, {
        username: 'chuck biff',
    ...
    test('determines if an object created with new Parse.User(...) -- and then saved -- is an instance of Parse.User', function () {
      return expect(user.signUp().then(_node4.default.isUser)).resolves.toBe(true);
    });
    test('determines if a pointer to a User is a not an instance of Parse.User', function () {
      expect(_node4.default.isUser(user.toPointer())).toBe(false);
    });
    test('determines if a object is a not an instance of Parse.User', function () {
      expect(_node4.default.isUser(unsavedParseObj)).toBe(false);
    });
  });

  describe('classStringOrSpecialClass', function () {
    test('converts the unchanged string if not a special Parse class', function () {
      expect(_node4.default.classStringOrSpecialClass('Horse')).toBe('Horse');
    });
    test('converts "User" to Parse.User', function () {
      expect(_node4.default.classStringOrSpecialClass('User')).toBe(_node2.default.User);
    });
    test('converts "Session" to Parse.Session', function () {
      expect(_node4.default.classStringOrSpecialClass('Session')).toBe(_node2.default.Session);
    });
    test('converts "Role" to Parse.Role', function () {
      expect(_node4.default.classStringOrSpecialClass('Role')).toBe(_node2.default.Role);
    });
  });
});